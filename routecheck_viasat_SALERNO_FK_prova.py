...
# cur_HAIG.execute("DROP TABLE IF EXISTS routecheck_2017_temp CASCADE")
# conn_HAIG.commit()
...
########################################################################################
    ...
    if (len(viasat_data) > 5) and sum(viasat_data.progressive)> 0:  # <----
        fields = ["new_id", "longitude", "latitude", "progressive", 'panel', 'grade', "timedate", "speed"]
        ...
        if len(viasat) > 0:
            ...
            viasat = viasat[["new_id", "longitude", "latitude", "progressive", "path_time", "totalseconds",
            ...
            if len(row)>0:
                ...
                for idx, j in enumerate(row):
                    ...
                    if j == row[0]:  # first TRIP
                        ...
                        viasat['idtrajectory'] = viasat.new_id.iloc[0]
                        # print(viasat)
                    ...
                    if j == "end":  # last trip for that ID
                        ...
                        for idx, j in enumerate(row_progr):
                            ...
                            idtrajectory = str(VIASAT_TRIPS_by_ID['new_id'].iloc[j])
                    ...
                    #############################################
                        ...
                        for idx_trip, trip in enumerate(all_TRIPS):
                          ...
                          ####### TRAVEL time > 10 min ###############################################
                            ...
                            VIASAT_TRIP.drop(["longitude", "latitude", "progressive", 'panel', 'grade',
                                              "timedate", "hour",

                                              'last_panel', 'last_lon', 'last_lat',
                                              'last_totalseconds', 'last_progressive'], axis=1,
                                             inplace=True)

                            #### Connect to database using a context manager and populate the DB ####
                            connection = engine.connect()
                            VIASAT_TRIP.to_sql("routecheck_2017_temp", con=connection, schema="public",
                                               if_exists='append')
                            connection.close()
