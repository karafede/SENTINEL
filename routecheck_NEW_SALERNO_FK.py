...
## create extension postgis on the database HAIG_SALERNO  (only one time)
...
all_VIASAT_IDterminals = pd.read_sql_query(
    ''' SELECT idterm
        FROM public.dataraw
        /*WHERE EXTRACT(YEAR FROM dataraw.timedate) = '2019'*/   ## only year 2019
         ''', conn_HAIG)

# make a list of all IDterminals (GPS ID of Viasata data) each ID terminal (track) represent a distinct vehicle
all_ID_TRACKS = list(all_VIASAT_IDterminals.idterm.unique())
with open("D:/ENEA_CAS_WORK/SENTINEL/viasat_data/all_idterms.txt", "w") as file:
    file.write(str(all_ID_TRACKS))

"""

## reload 'all_ID_TRACKS' as list
with open("D:/ENEA_CAS_WORK/SENTINEL/viasat_data/all_idterms_new.txt", "r") as file:
...
# for last_track_idx, track_ID in enumerate(all_ID_TRACKS):
    ...
    viasat_data = pd.read_sql_query('''
                ...
                WHERE idterm = '%s'
                /* and EXTRACT(YEAR FROM dataraw.timedate) = '2019' */
                ''' % track_ID, conn_HAIG)
...
if __name__ == '__main__':
    ...
    pool = mp.Pool(processes=55)     ## use 45 processors
    print("++++++++++++++++ POOL +++++++++++++++++", pool)
