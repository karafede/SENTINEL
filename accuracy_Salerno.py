...
# make a list of all unique trips
all_TRIP_IDs_routecheck = list(all_VIASAT_TRIP_IDs.TRIP_ID.unique())
...
# with open("D:/ENEA_CAS_WORK/SENTINEL/viasat_data/all_TRIP_IDs_2019.txt", "r") as file:
...
### parallel processing to calculate the accuracy

def func(arg):
    last_trip_idx, trip = arg
...
if __name__ == '__main__':
    ...
    print("++++++++++++++++ POOL +++++++++++++++++", pool)
    results = pool.map(func, [(last_trip_idx, trip) for last_trip_idx, trip in enumerate(all_TRIP_IDs)])
    pool.close()
    pool.close()
    pool.join()
