...
q <- ggplot(data = total_counts_direzione_type,
  ...
  ggtitle("Numero di passaggi totali (1-18 Sett. 2017)") +
...
p <- ggplot(data = VIASAT_STATS_passaggi_daily,
    ...
    ggtitle("Numero totale di passaggi per giorno della settimana (VIASAT, 1-18 Sett. 2017)") + 
...
p <- ggplot(data = VIASAT_STATS_passaggi_hourly,
    ...
    ggtitle("Numero totale di passaggi orari nei giorni feriali (1-18 Sett. 2017)") + 
    theme(plot.title = element_text(lineheight=.8, face="bold"))
  p
...
p <- ggplot(data = VIASAT_STATS_passaggi_hourly,
    ...
    theme(axis.title.y = element_text(face="bold", colour="black", size=12),
          axis.text.y  = element_text(angle=0, vjust=0.5, size=12)) +
    scale_x_continuous(breaks=c(0, 5, 10, 15, 20)) +
    ggtitle("Numero totale di passaggi orari nei giorni festivi (1-18 Sett. 2017)") + 
    theme(plot.title = element_text(lineheight=.8, face="bold"))
  p
...
q <- ggplot(data = portata_veh, 
  ...
  ggtitle("Conteggi dei principali veicoli pesanti per valore della loro portata (1-18 Sett. 2017)") + 
...
q <- ggplot(data = portata_veh, 
  ...
  ylim(0, 2200) +
   theme(axis.title.y = element_text(face="bold", colour="black", size=13),
        axis.text.y  = element_text(angle=0, vjust=0.5, size=13, colour="black")) +
  ggtitle("Conteggi dei veicoli pesanti per valore della loro portata (1-18 Sett. 2017)") + 
  theme(plot.title = element_text(lineheight=.8, face="bold", size = 15))
# q
...
all_superamenti <- sum(superamenti$count_superamenti)
# all_portata_veh$total
all_NO_superamenti <- all_portata_veh$total - all_superamenti

count_superamenti <- data.frame(veicoli_pesanti = c(all_NO_superamenti, all_superamenti), 
                                superamenti =c("", "superamenti"),
                                perc_superamenti = c("0", round(((all_superamenti)/(all_portata_veh$total))*100, digits = 0)))
count_superamenti$classi <- "veicoli pesanti"

perc_superamenti <- count_superamenti$perc_superamenti[2]
...
q1 <- ggplot(data = portata_veh,
            aes(classi, conteggi, fill = `portata (kg)`)) +
  theme_bw() +
  geom_bar(stat = "identity", position = position_stack()) +
   # guides(fill=FALSE)
  scale_x_discrete(limits = categories) +
...
q2 <- ggplot(data = count_superamenti,
            aes(classi, veicoli_pesanti, fill = veicoli_pesanti)) +
  theme_bw() +
  geom_bar(stat = "identity", position = position_stack()) +
  guides(fill=FALSE) +
  scale_x_discrete(limits = categories) +
...
grid.arrange(q1, q2, nrow=1, ncol = 2,
             top = textGrob("              passaggi (84%) per portata                                         passaggi (100%) - superamenti limite velocità",gp=gpar(fontsize=13,font=3)))
...
q <- ggplot(data = superamenti_by_portata, 
  ...
  ylim(0, 500) +
   theme(axis.title.y = element_text(face="bold", colour="black", size=13),
        axis.text.y  = element_text(angle=0, vjust=0.5, size=13, colour="black")) +
  ggtitle("Numero di superamenti limite velocità per valore della loro portata (1-18 Sett. 2017)") + 
  theme(plot.title = element_text(lineheight=.8, face="bold", size = 15))
q
