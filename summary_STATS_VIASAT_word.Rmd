...
Come ripotato in **Tabella 3**, il mumero totale di passaggi di dati Viasat rappresenta circa il **`r penetration`%** dei dati ANAS su entrambe le direzioni di marcia.
...
q <- ggplot(data = total_counts_direzione_type,
  ...
  geom_text_repel(aes(label=`frazione %`), vjust=1, color="black",
...
Dall'analisi dei valori di portata massima dei veicoli pesanti è stato osservato un valore minimio di 1180 kg ed un valore massimo di 39000 kg. Tuttavia, come illustrato in **Figura 5**, circa il 55% dei veicoli pesanti ha una portata massima di 18000 kg seguiti dall'11% e dal 7.5% con una portata di 3500 kg e 26000 kg, rispettivamente.


*<br/><br/>*

````{r, message = FALSE, echo = FALSE, warning = FALSE, fig.width=9,fig.height=6,  fig.cap ="**Figura AAA.** Classificazione dei veicoli pesanti per *portata* su un tratto dell'autostrada A2 (Salerno-Avellino)."}
...
q <- ggplot(data = portata_veh, 
    ...
    theme_bw() +
   geom_line(alpha=1, col="red", size = 1) +
   geom_point(alpha=1, col="red", size = 2) +
    guides(fill=FALSE) +
...
````{r, message = FALSE, echo = FALSE, warning = FALSE, fig.width=9,fig.height=5,  fig.cap ="**Figura BBB.** ."}
...
count_superamenti <- data.frame(veicoli_pesanti = c(all_NO_superamenti, all_superamenti), 
                                ...
                                perc_superamenti = c("", round(((all_superamenti)/(all_portata_veh$total))*100, digits = 0)))
...
q <- ggplot(data = portata_veh,
            aes(classi, conteggi, fill = portata)) +
  theme_bw() +
  geom_bar(stat = "identity", position = position_stack()) 
   # guides(fill=FALSE)
  q

q <- ggplot(data = count_superamenti,
