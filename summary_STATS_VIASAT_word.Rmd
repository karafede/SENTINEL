...
# data <- read.csv(paste0("FCD_2017_UNISA_2019.csv"),
#                                    header = T, sep=",")[-1]
...
p <- ggplot(joined_ANAS_VIASAT_grezzi, aes(x=ratio, fill=direzione)) +
    ...
    xlab("coefficiente penetrazione (%) (ANAS / VIASAT)") +
...
p1 <- ggplot(joined_ANAS_VIASAT_PHF, aes(x=ratio, fill=direzione)) +
     geom_histogram() +
     theme_bw() +
    # geom_density(stat = 'bin') +
    theme(legend.title=element_blank()) + 
...
# #### get coefficiente di penetrazione (VIASAT-ANAS) by hour 
# 
# counts_direzione_all <- data %>%
#     group_by(Date=floor_date(timedate, "1 hour"), direzione, hour) %>%
#     summarize(count = length(idterm))
# 
# 
# counts_VIASAT_hourly <- counts_direzione_all %>%
#     group_by(direzione,
#              hour) %>%
#     summarize(count_VIASAT = sum(count))
# 
# 
# ## load hourly ANAS counts
# ### percentage of veicoli pesanti e auto (ANAS)
# counts_ANAS_hourly <- read.csv("D:/ENEA_CAS_WORK/SENTINEL/ANAS/ALL_passaggi_hourly_ANAS.csv")[-1]
# counts_ANAS_hourly$hour <- as.factor(counts_ANAS_hourly$hour)
# counts_ANAS_hourly$conteggi_totali <- round(counts_ANAS_hourly$conteggi_totali, digits = 0)
# 
# 
# counts_ANAS_hourly <- counts_ANAS_hourly %>%
#   left_join(counts_VIASAT_hourly, by = c("direzione", "hour"))
# counts_ANAS_hourly$count_VIASAT <- round(counts_ANAS_hourly$count_VIASAT, digits = 0)
# 
# counts_ANAS_hourly$fraction <- round(((counts_ANAS_hourly$count_VIASAT)/counts_ANAS_hourly$conteggi_totali)*100, digits = 1)
# 
# 
# counts_ANAS_hourly$hour <- as.numeric(counts_ANAS_hourly$hour)
# 
# #### plot
# 
# 
# 
# p <- ggplot(data = counts_ANAS_hourly,
#               aes(hour, fraction, fill = fraction)) + guides(fill=FALSE) +
...
#     ylab("passaggi VIASAT / ANAS (%)") +            # Set y-axis label
...
#    geom_hline(yintercept=3, color="red", linetype="dashed", size=0.5) +
#    geom_hline(yintercept=4, color="red", linetype="dashed", size=0.5) +
#     ggtitle("Coefficiente di penetrazione VIASAT-ANAS (31 Ago.-18 Sett. 2017)") + 
...
Tuttavia il coefficiente di penetrazione VIASAT / ANAS calcolato su sul dataset completo di dati rappresenta un **limite superiore**. Infatti, le distribuzioni orarie dei conteggi ANAS/VIASAT, mostranto un **limite inferiore** dell'**1.0% - 1.3%** per le automobili ed i veicoli pesanti, rispettivamente (**Figura A1, Figura A2, Figura A3**).
...
````{r, message = FALSE, echo = FALSE, warning = FALSE, fig.width=9,fig.height=5,  fig.cap ="**Figura 5a.** Diagrammi riassuntivi per il numero di passaggi per portata e per il numero totale di superamenti dei limiti di velocità."}
...
````{r, message = FALSE, echo = FALSE, warning = FALSE, fig.width=9,fig.height=5,  fig.cap ="**Figura 6.** Distribuzione delle velocita' istantanee per classe di autoveicoli su un tratto dell'autostrada A2 (31 Ago.-18 Sett. 2017; Salerno)"}
...
````{r, message = FALSE, echo = FALSE, warning = FALSE, fig.width=9,fig.height=5,  fig.cap ="**Figura 6a.** Distribuzione delle velocita' istantanee per classe di autoveicoli su un tratto dell'autostrada A2 (31 Ago.-18 Sett. 2017; Avellino)"}
...
I box-plots riportati in **Figura 8** mostrano che i dati VIASAT hanno una variabilita' piu' grande rispetto ai dati ANAS. Inoltre, I dati VIASAT presentano molti valori estremi ed anche un maggior numero di *outliers* riseptto ai dati ANAS. Malgrado queste differenze, gli interquartili delle due distribuzioni sono molto simili.
