...
````{r, message = FALSE, echo = FALSE, warning = FALSE, fig.width=9,fig.height=5,  fig.cap ="**Figura AAA.** Flusso per ora della giornata su un tratto dell'autostrada A2 (1-18 Settembre 2017; Salerno-Avellino)."}
...
VIASAT_STATS_passaggi_hourly <- data %>%
  ...
  group_by(Date=floor_date(timedate, "15 minute"), direzione, day, hour) %>%
  summarize(instant_speed=mean(speed),
              count = length(idterm))
...
p <- ggplot(data = VIASAT_STATS_passaggi_hourly,
    ...
    theme(axis.title.y = element_text(face="bold", colour="black", size=12),
          axis.text.y  = element_text(angle=0, vjust=0.5, size=12)) +
    scale_x_continuous(breaks=c(0, 5, 10, 15, 20)) +
    ggtitle("FLusso orario (1-18 Settembre 2017)") + 
    theme(plot.title = element_text(lineheight=.8, face="bold"))
  p
...
````{r, message = FALSE, echo = FALSE, warning = FALSE, fig.width=9,fig.height=5,  fig.cap ="**Figura BBB** Desnsità per ora su un tratto dell'autostrada A2 (1-18 Settembre 2017; Salerno-Avellino)."}
...
density_by_day$density <- ((density_by_day$count)/((density_by_day$length)/1000)  )

density_by_day$hour <- as.numeric(density_by_day$hour)

p <- ggplot(data = density_by_day,
              aes(hour, density, fill = hour)) + guides(fill=FALSE) +
    geom_bar(stat = "identity") + 
    facet_wrap( ~ direzione, ncol = 4) +
    # facet_grid(direzione ~ vehtype, scales = "free", space = "free") +
    guides(fill=FALSE) +
    ...
    theme(axis.title.y = element_text(face="bold", colour="black", size=12),
          axis.text.y  = element_text(angle=0, vjust=0.5, size=12)) +
    scale_x_continuous(breaks=c(0, 5, 10, 15, 20)) +
    ggtitle("Densità spaziale (1-18 Settembre 2017)") + 
    theme(plot.title = element_text(lineheight=.8, face="bold"))
  p
...
ks.test(velocita_ANAS$speed, data$speed)


## Wilcoxon-Manney test)
wilcox.test(velocita_ANAS$speed, data$speed, var.equal=TRUE)
