...
I box-plots riportati in **Figura 7** mostrano che i dati VIASAT hanno una variabilita' piu' grande rispetto ai dati ANAS. Inoltre, I dati VIASAT presentatno molti valori estremi ed anche un maggior numero di *outliers* riseptto ai dati ANAS. Malgrado queste differenze, gli interquartili delle due distribuzioni sono molto simili.
Il t-test confronta la differenza osservata fra le medie delle due distribuzioni (ANAS e VIASAT). Se la differenza è maggiore di 0, allora si puo' parlare di indipendenza statistica tra le due distribuzioni.
 Poichè la differenza fra le medie è **`r tvalue` km/h**, l’ipotesi nulla è quindi da scartare e la differenza tra le distribuzioni è compresa fra −7.0 e 8.4 km/h (l’ipotesi nulla è che la differenza sia uguale 0). Inoltre il valore di p è **`r pvalue`** e quindi i dati osservati sono statisticamente significativi.
...
Per convenzione, il flusso orario dei veicoli è stato calcolato su una base temporale di **15 minuti**. Tuttavia, essedo i veicoli VIASAT solo una frazione dei veicoli misurati da ANAS, abbiamo abbiamo stimato il conteggio dei veicoli VIASAT applicando il fattore di rappresentatività riportato in Tabella 2. La densità spaziale, intesa come il numero d veicoli nella sezione stradale in esame, e' stata stimata per ogni ora del giorno. Il rapporto tra il flusso orario e la velocità è risultato nella stima della densità che è rappresentata in **Figura 10**.  
...
````{r, message = FALSE, echo = FALSE, warning = FALSE, fig.width=9,fig.height=5,  fig.cap ="**Figura 10.** Densità spaziale (veicoli/km) stimata per ogni ora della giornata su un tratto dell'autostrada A2 (1-18 Settembre 2017; Salerno-Avellino)."}
...
*<br/><br/>*
Gli stati del traffico sono stati stimati usando le procedure riportate nell'*Highway capacity manual. Washington, D.C: Transportation Research Board, National Research Council. National Research Council (U.S.) (Ed.). (2000)*. Per quanto concerne le autostrade,gli stati del traffico per ogni ora della giornata e direzioni di marcia sono stati stimati attraverso la densita' veicolare.

LIVELLI DI SERVIZIO: è definito come
misura della prestazione della strada
nello smaltire traffico.
I livelli di servizio (LOS – Level Of
Service) sono 6 da LOS “A” a LOS “F”.
I LOS da “A” a “D” hanno una densità
inferiore a quello corrispondente alla
capacità.
Il LOS E corrisponde alla densità critica
e quindi alla capacità della strada.
Il LOS “F” ha densità maggiori e quindi
siamo in presenza di flusso instabile.

LOS “A”: circolazione libera, massimo comfort, flusso stabile (Densità
≤ 7 veic/km/cosia);
LOS “B” circolazione libera, modesta riduzione della velocità, comfort accettabile, flusso
stabile (7 < Densità
≤ 11 veic/km/corsia);
LOS “C”: primi vincoli alla circolazione, riduzione velocità, si riduce il comfort, flusso
stabile (11 < Densità
≤ 16 veic/km/corsia);
LOS “D”: vincoli alla circolazione, basso comfort, flusso che può iniziare ad essere
instabile (16 < Densità
≤ 22 veic/km/corsia);


````{r, message = FALSE, echo = FALSE, warning = FALSE, fig.width=9,fig.height=5,  fig.cap ="**Figura 11.** FLusso orario (veicoli/ora) e stati del traffico calcolati sul tratto dell'autostrada A2 (1-18 Settembre 2017; Salerno-Avellino). Le lettere **A** e **B** indicano uno stato **LIBERO**, mentre la lettera **C** indica un stato **STABILE**."}
