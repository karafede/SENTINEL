...
````{r, message = FALSE, echo = FALSE, warning = FALSE, fig.width=9,fig.height=5,  fig.cap ="**Figura 5a.** Numero di passaggi totali per classe di autoveicoli sul tratto dell'autostrada A2 (km5+004) per il periodo 31/08/2017 - 18/09/2017 (come dati FCD Viasat). I numeri all'interno del grafico indicano la distribuzione percentuale per classi di veicolo del numero di passaggi sulle due direzioni di marcia: --> Salerno e <-- Avellino."}
...
# get all names for specific classes
all_passaggi <- names(DB_selected[grep(pattern  = "passaggi", x = names(DB_selected))])
all_passaggi_idx <- which(names(DB_selected) %in%  names(DB_selected[grep(pattern  = "passaggi", x = names(DB))]) )
# select only "passaggi"
DB_passaggi <- DB_selected[all_passaggi]
# add corsia and direzione
DB_passaggi <- cbind(DB_passaggi, DB_selected$corsia, DB_selected$direzione)
names(DB_passaggi) <- c(all_passaggi, "corsia", "direzione")
# transpose data
passaggi <- gather(DB_passaggi, "classi", "records", 1:length(all_passaggi))
passaggi$records <- as.numeric(passaggi$records)
...
# only keep the "conteggi" by classe
STATS_passaggi <- STATS_passaggi %>%
  filter(!classi %in% c("N_conteggio", "N_altri", "N_auto_rimorchio", "N_moto")) %>%
  filter(classi %in% to_keep) %>%
  summarise(conteggi_totali = sum(conteggi_totali))
...
velocita_ANAS <- DB %>%
  filter(date < max & date > min) %>% 
  select(date, velocita__media__km_h_)
